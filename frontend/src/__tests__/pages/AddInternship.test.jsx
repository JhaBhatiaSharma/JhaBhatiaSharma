...
describe('AddInternship', () => {
  ...
  test('renders the form with all fields', () => {
    ...
    expect(screen.getByPlaceholderText('Duration (in months)')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('Stipend (in USD)')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('Add a required skill')).toBeInTheDocument();
    
    // Check buttons
    expect(screen.getByRole('button', { name: 'Cancel' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Post Internship' })).toBeInTheDocument();
  ...
  test('handles form input changes', () => {
    ...
    fireEvent.change(titleInput, { target: { value: 'Software Engineer Intern' } });
  ...
  test('validates duration input', () => {
    ...
    expect(screen.getByText('Duration cannot exceed 24 months')).toBeInTheDocument();

    // Test valid duration
    fireEvent.change(durationInput, { target: { value: '6' } });
    expect(screen.queryByText('Duration cannot exceed 24 months')).not.toBeInTheDocument();
  });
  ...
  test('validates stipend input', () => {
    ...
    expect(screen.getByText('Stipend must be a non-negative number')).toBeInTheDocument();

    // Test valid stipend
    fireEvent.change(stipendInput, { target: { value: '1000' } });
    expect(screen.queryByText('Stipend must be a non-negative number')).not.toBeInTheDocument();
  });
  ...
  test('handles form submission errors', async () => {
    ...
    // Submit the form
    await act(async () => {
      const submitButton = screen.getByRole('button', { name: 'Post Internship' });
