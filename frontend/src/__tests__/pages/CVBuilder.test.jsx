...
describe("CVBuilder", () => {
  ...
  test("renders nothing when not open", () => {
    const { container } = render(
      <CVBuilder isOpen={false} onClose={mockOnClose} />,
    );
  ...
  test("allows adding and removing skills", async () => {
    ...
    const skillsSection = screen
      .getAllByRole("heading", { level: 3 })
      .find((h) => h.textContent === "Skills")?.parentElement;
  ...
  test("allows template switching", async () => {
    ...
    const professionalBtn = screen.getByRole("button", {
      name: "Professional",
    });
  ...
  test("allows adding education entries", async () => {
    ...
    const initialCount =
      screen.getAllByPlaceholderText("School/University").length;

    await act(async () => {
      fireEvent.click(addButton);
    });

    expect(screen.getAllByPlaceholderText("School/University")).toHaveLength(
      initialCount + 1,
    );
  });
});
