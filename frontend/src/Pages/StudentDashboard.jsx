...
const StudentDashboard = () => {
  ...
  const [showActiveInternshipsModal, setShowActiveInternshipsModal] =
    useState(false);
  ...
  const fetchCompletedInterviews = async () => {
    try {
      const response = await API.get(
        "/internships/student/completed-interviews",
        {
          headers: {
      ...
      console.error(
        "Error fetching completed interviews:",
  ...
  useEffect(() => {
    ...
    const fetchActiveInternships = async () => {
      try {
        ...
        console.error(
          "Error fetching active internships:",
          error.response?.data?.message || error.message,
        );
      }
    };
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-[#666]">
            Find Suitable Internships and Apply with ease
          </p>
        </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-sm font-medium ${getMatchScoreColor(internship.matchScore)}`}
                          >
            ...
            <div className="mb-6">
              ...
              <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-700">
                  Your profile has a {Math.round(selectedInternship.matchScore)}
                  % match with this role
                </p>
                {selectedInternship.matchingSkills?.length > 0 && (
                  <p className="text-sm text-blue-600 mt-2">
                    Matching skills:{" "}
                    {selectedInternship.matchingSkills.join(", ")}
                  </p>
                )}
              </div>
            </div>
